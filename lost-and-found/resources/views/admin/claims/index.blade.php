...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600">Review and process user claims</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $claims->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $claims->where('status', 'pending')->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $claims->where('status', 'approved')->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $claims->where('status', 'rejected')->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="sm:hidden mb-6">
        ...
        <select id="tabs" name="tabs" onchange="location = this.value;"
                class="block w-full focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
            <option value="{{ route('admin.claims.index') }}" {{ !request('status') ? 'selected' : '' }}>All Claims</option>
            <option value="{{ route('admin.claims.index', ['status' => 'pending']) }}" {{ request('status') == 'pending' ? 'selected' : '' }}>Pending</option>
            <option value="{{ route('admin.claims.index', ['status' => 'approved']) }}" {{ request('status') == 'approved' ? 'selected' : '' }}>Approved</option>
            <option value="{{ route('admin.claims.index', ['status' => 'rejected']) }}" {{ request('status') == 'rejected' ? 'selected' : '' }}>Rejected</option>
        </select>
    </div>
        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8">
                <a href="{{ route('admin.claims.index') }}"
                   class="{{ request('status') ? 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' : 'border-indigo-500 text-indigo-600' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm">
                    All Claims
                </a>
                <a href="{{ route('admin.claims.index', ['status' => 'pending']) }}"
                   class="{{ request('status') == 'pending' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm">
                    Pending ({{ $claims->where('status', 'pending')->count() }})
                </a>
                <a href="{{ route('admin.claims.index', ['status' => 'approved']) }}"
                   class="{{ request('status') == 'approved' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Claims List</h3>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Review and manage all submitted claims</p>
        </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-8 w-8 bg-gray-300 rounded-full flex items-center justify-center">
                                            <span class="text-sm font-medium text-gray-700">{{ substr($claim->user->username, 0, 1) }}</span>
                                        </div>
                                        <div class="ml-3">
                                            <div class="text-sm font-medium text-gray-900">{{ $claim->user->username }}</div>
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">No claims have been submitted yet.</p>
            </div>
        @endif
    </div>
</div>
