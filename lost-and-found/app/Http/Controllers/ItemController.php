...
class ItemController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        return view('items.search', compact('items', 'categories', 'locations'));
    }
    ...
    public function createLost()
    {
        ...
        return view('items.lost.create', compact('categories', 'locations')); // <-- CORRECTED
    }
    ...
    public function createFound()
    {
        ...
        return view('items.found.create', compact('categories', 'locations')); // <-- CORRECTED
    }
    ...
    public function storeLost(Request $request)
    {
        $validated = $request->validate([
            ...
            'location' => ['required', 'string', Rule::in($this->getLocations())], 
            'date_lost_found' => 'required|date|before_or_equal:today',
            'contact_info' => 'nullable|string|max:500',
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
            'reward_offered' => 'boolean',
            'reward_amount' => 'nullable|numeric|min:0|max:999999.99|required_if:reward_offered,true', 
        ]);
    ...
    public function storeFound(Request $request)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'description' => 'required|string|max:1000',
            'category' => ['required', 'string', Rule::in(array_keys($this->getCategories()))], // Use Rule::in with keys
            'location' => ['required', 'string', Rule::in($this->getLocations())], // Use Rule::in
            'date_lost_found' => 'required|date|before_or_equal:today',
            'contact_info' => 'nullable|string|max:500',
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
